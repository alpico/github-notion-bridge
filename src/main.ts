...
import contextObject from './sample_events/deleted.json';
...
async function handleIssuesEvent(context: Context): Promise<void> {
  switch (context.payload.action as IssuesAction) {
    ...
    case "edited": await issues.edit(context)
    case "deleted": await issues.deletePage(context)
    ...
    case "assigned": await issues.assign(context)
    case "unassigned": await issues.unassign(context)
    case "labeled": await issues.label(context)
    case "unlabeled": await issues.unlabel(context)
